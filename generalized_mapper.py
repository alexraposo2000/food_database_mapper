...
def mapping_function(df1,col1,df2,col2,df3 = None,threshold = 0.5,methods= ["fuzzy","tf-idf","BERT"],dataset_name = ""):
    ...
    for method in methods:
        if method == 'BERT':
            def run_remote_function(function_name, input_data):
                # Define server details
                hostname = 'ceres.scinet.usda.gov'
                username = 'aeraposo'
                password = 'Snails175!!!'  # Use SSH key instead if possible
                ...
                # Command to run the specific function with input
                command = f'python /database_mapper/BERT_match.py {function_name} "{input_data}"'
                ...
                return output, error
            function_name = 'bert_match'
            input_data = [df1,df2]
            
            output, error = run_remote_function(function_name, input_data)
